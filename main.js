...
obj_1.getInfo = function userInfo () {
	console.log(`${this.registered}` !== false ? "Дата регистрации: " + `${this.data}` : "Незарегистрированный пользователь: " + `${this.name}` )
}

obj_2.getInfo = function userInfo () {
	console.log(`${this.registered}` === true ? "Дата регистрации: " + `${this.data}` : "Незарегистрированный пользователь: " + `${this.name}` )
}
...
function getCurrentPostComments ( postId ) {
	...
	for(var id in comments) {
		if (comments[id].postId !== postId) continue
			res.push({
				author: users[comments[id].author].name, text: comments[id].text
			})
	}
